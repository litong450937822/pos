...
function calculatePromotions(inputs) {
  let promotionsItems = new Promotion
  promotionsItems = promotionsItems.constructor.all()
  promotionsItems =promotionsItems[0].barcodes
  inputs.forEach(elemtne=>{
    if(promotionsItems.some(item=>item===elemtne.barcode))
      elemtne.discount=Math.floor(parseFloat(elemtne.count)/3)
...
function buildCartItems(inputs) {
  ...
  allItems = allItems.constructor.all()
